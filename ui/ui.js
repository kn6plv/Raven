...
function htmlChannelConfig()
{
    const body = echannels.map((e, i) => {
        ...
        return `<form class="c">
            ...
            <input value="${e.meshtastic ? e.name : toDisplayKey(e.key)}" oninput="typeChannelKey(${i}, event.target)" required minlength="4" maxlength="44" size="44" placeholder="ID or Key" ${e.aredn || e.meshtastic || e.meshcore || (e.name[0] === "#" || e.name[0] === "%") ? "disabled" : ""}>
